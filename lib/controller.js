...
Babylon.Controller.prototype.render_and_send = function(options){
  ...
  var stanza          = Babylon.Stanzas.get(options.controller, options.action_name);
  var data            = stanza(options.data);
  Babylon.log.info("Sending from render_and_send");
  Babylon.Runner.connection.send(data);
};
...
Babylon.Controller.prototype.render_with_callbacks = function(options) {
  ...
  Babylon.Runner.connection.sendIQ(data, options);
};

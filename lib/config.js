...
Babylon.config = {
  ...
  nickname: function(){
    return Babylon.config.username();
  },

  thumbIcon: function(){
    return '/users/' + Babylon.config.username() +'/icon/thumb';
  },

  profilePath: function(){
    return '/users/' + Babylon.config.username();
  },
  ...
  full_jid: function(){
    ...
    return $.map([Babylon.config._options._bare_jid, Babylon.config._options._resource], function(i){ return i; } ).join("/");
  },
  ...
  muc_domain: function(){
    Babylon.config.log_error_if_blank("session_bot", ['_domain']);
    return 'conference.' + Babylon.config._options._domain;
  },

  muc_domain_with_user: function(){
    Babylon.config.log_error_if_blank("session_bot", ['_domain']);
    return this.muc_domain() + '/' + this.username();
  },

  muc_domain_with_user_and_resource: function(){
    Babylon.config.log_error_if_blank("session_bot", ['_domain']);
    return this.muc_domain_with_user() + '_____' + this.resource();
  },
  ...
  is_tutor: function(){
    return $('meta[name="is-tutor"][content="true"]').length > 0;
  },

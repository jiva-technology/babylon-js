...
Babylon.Router.prototype.route = function(stanza) {
  ...
  var routes = this.routes.filter(function(r, i, a) { return r.accepts(stanza); });
  ...
  if(routes.length >= 1){
    ...
    Babylon.log.info('No route matches this stanza');
    return false;
  }
};
...
Babylon.Router.prototype.execute_route = function(controller, action, stanza) {
  ...
  // don't want to blindly call send as most of the time when we
  // receive a message there is no message to be send back immediatly

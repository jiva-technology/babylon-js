...
Babylon.Connection.prototype.connect = function(jid, password) {
  ...
  var that = this;
  this.connection.connect(credentials.full_jid, this.password, function(s,e){that.on_connect(s,e);});
};
...
Babylon.Connection.prototype.find_cookie = function(){
  ...
  if(Babylon.config.attach && cookie && cookie !== ""){
    this.set_credentials(Strophe.getBareJidFromJid(cookie.jid));
    return cookie;
  }else{
    return false;
  }
...
Babylon.Connection.prototype.warm_and_attach = function() {
  ...
  $.post('/session/warm', function(data) {
    var xml = $.xmlDOM(data);
    var jid = xml.find(".binding dd.jid").text();
    var sid = xml.find(".binding dd.sid").text();
    var rid = xml.find(".binding dd.rid").text();

    console.log(">>>>>>>>>>>>>>>> warmed sid:" + sid + ", rid:" + rid + " for " + jid);

    that.reattach(jid, sid, rid);
  });
};

Babylon.Connection.prototype.set_credentials = function(jid, password) {
  var name = jid.split("@")[0];
  var domain = Babylon.config.domain;
  Babylon.config.jid = name + "@" + domain;
  Babylon.config.full_jid = Babylon.config.jid + '/' + Babylon.config.resource;
  if(password){
...
Babylon.Connection.prototype.reattach = function(jid, sid, rid) {
  ...
  this.connection.attach(jid, sid, rid, function(s,e){that.on_connect(s,e);});
};

...
Babylon.Connection.prototype.warm_and_attach = function() {
  ...
  $.post('/session/warm.json', {}, function(data) {
    Babylon.log.info(">>>>>>>>>>>>>>>> warmed sid:" + data.sid + ", rid:" + data.rid + " for " + data.jid);
...
Babylon.Connection.prototype.reconnect_or_destroy_session = function() {
  this.erase_cookie();
  if(Babylon.config.reconnect() && this.reconnect_count < this.reconnect_limit){
    this.reconnect_count++;
    if(Babylon.config.attach()){
      this.warm_and_attach();
      return;
    }else if(Babylon.config.password()){
      this.connect(Babylon.config.full_jid(), Babylon.config.password());
      return;
    }
  }
  this.unregister_cookie_callback();
...
Babylon.Connection.prototype.read_cookie = function(expires) {
  if (Babylon.Connection.cookie === undefined){
    Babylon.Connection.cookie = $.cookie("babylon");
  }
  if (Babylon.Connection.cookie && Babylon.Connection.cookie !== "") {
    var s = Babylon.Connection.cookie.split(",");
...
Babylon.Connection.prototype.send = function(stanza) {
  Babylon.log.info("Sending: " + Strophe.serialize(stanza));

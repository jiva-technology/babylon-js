...
Babylon.Connection.prototype.rawInput = function(data) {
  if(data.search(/id='ping'/) < 0){ // only interested in message that are not pings
...
Babylon.Connection.prototype.warm_and_attach = function() {
  ...
  $.ajax({
    ...
    error:    function(request, textStatus, errorThrown){
                ...
                if(request.status != 410){ that.reconnect_or_destroy_session(); }
              },
    dataType: 'json'
  });
};
...
Babylon.Connection.prototype.send_presence = function(){
  this.send($pres().c('priority').t('5').tree());
};
